/**
 * 
 */

function createPdfAit(numAuto,placa, marcaModelo, cor, tipo, cpfCnpjProprietario, especie, endereco, municipio, sentido, pontoReferencia, dataHora, baseLegal, enquadramento, desdobramento, descricaoInfracao, limiteRegulamentar,medicaoRealizada, medicaoConsiderada, excessoVerificado, equipamentoUtilizado, ultimaAfericao, docTransportadorEmbarcador, nomeTransportadorEmbarcador, cpfCondutor, nomeCondutor, habilitacaoCondutor, ufHabilitacaoCondutor){
	console.log(numAuto);
	console.log(1);
	console.log(placa);
	console.log(marcaModelo);
	console.log(cor);
	console.log(tipo);
	console.log(cpfCnpjProprietario);
	console.log(especie);
	console.log(2);
	console.log(endereco);
	console.log(municipio);
	console.log(sentido);
	console.log(pontoReferencia);
	console.log(dataHora);
	console.log(3);
	console.log(baseLegal);
	console.log(enquadramento);
	console.log(desdobramento);
	console.log(descricaoInfracao);
	console.log(limiteRegulamentar);
	console.log(4);
	console.log(medicaoRealizada);
	console.log(medicaoConsiderada);
	console.log(excessoVerificado);
	console.log(equipamentoUtilizado);
	console.log(ultimaAfericao);
	console.log(5);
	console.log(docTransportadorEmbarcador);
	console.log(nomeTransportadorEmbarcador);
	console.log(6);
	console.log(cpfCondutor);
	console.log(nomeCondutor);
	console.log(habilitacaoCondutor);
	console.log(ufHabilitacaoCondutor);
}

function geraPdfAit(numAuto, placa, marcaModelo, cor, tipo, cpfCnpjProprietario, especie,endereco, municipio, sentido, pontoReferencia, dataHora, baseLegal, enquadramento, desdobramento, descricaoInfracao, limiteRegulamentar, medicaoRealizada, medicaoConsiderada, excessoVerificado, equipamentoUtilizado, ultimaAfericao, docTransportadorEmbarcador, nomeTransportadorEmbarcador, cpfCondutor, nomeCondutor, habilitacaoCondutor, ufHabilitacaoCondutor, descLotacao, enderecoLotacao, telefoneLotacao, orgaoDescricao, orgaoCodigo){
	var dd = 
	{
		content: 
		[
		{
			style: 'tableExample',
			table: 
			{
			    widths: [80, '*', '*','*'],
				body: 
				[
					[{rowSpan: 4, image: 'data:image/png;base64,', width:80, height:80} , {colSpan:3, rowSpan: 4, alignment:'center', text: `${orgaoDescricao} - ${descLotacao}\nAUTO DE INFRAÇÃO\nCÓDIGO DO ÓRGÃO AUTUADOR: ${orgaoCodigo}\n \nAIT ${numAuto}`}, '',''],
					['L2', '', '',''],
					['L3', '', '',''],
					['L4', '', '',''],	
				]
			}	
		},
		{
			style: 'tableExample',
			table: 
			{
			    widths: ['*', '*', '*'],
				body: 
				[
					[{colSpan:3, text: '01 - IDENTIFICAÇÃO DO VEÍCULO', fillColor: '#b0bec5', border: [false, false, false, true]},'', ''],
					
					[[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Placa',italics:true}],[{text: placa, alignment:'center'}] ] },layout: 'noBorders'}] ,

					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Marca/Modelo',italics:true}],[{text: marcaModelo, alignment:'center'}] ] },layout: 'noBorders'}], 

					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Cor',italics:true}],[{text:cor, alignment:'center'}] ] },layout: 'noBorders'}]],
					
					[[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Tipo',italics:true}],[{text: tipo, alignment:'center'}] ] },layout: 'noBorders'}] ,

					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'CPF/CNPJ',italics:true}],[{text:cpfCnpjProprietario, alignment:'center'}] ] },layout: 'noBorders'}], 

					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Espécie',italics:true}],[{text:especie, alignment:'center'}] ] },layout: 'noBorders'}] ],
				]
			}
		},
		{
			style: 'tableExample',
			table: 
			{
			    widths: ['*'],
			    heights: ['*'],
				body: 
				[
					[{colSpan:1,text: '02 - LOCAL DA INFRAÇÃO', fillColor: '#b0bec5', border: [false, false, false, true]}],
					
					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Endereço',italics:true}],[{text:endereco, alignment:'center'}] ] }, layout: 'noBorders'}, ]
	                
				]
			}
		},
		{
			style: 'tableExample',
			table: 
			{
			    widths: ['*', '*'],
			    heights: ['*'],
				body: 
				[
				    [[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Municipio/UF',italics:true}],[{text:municipio, alignment:'center'}] ] },layout: 'noBorders'}],

	                [{table: {widths: ['*'], heights:['*'],body: [[{text: 'Sentido',italics:true}],[{text:sentido, alignment:'center'}] ] },layout: 'noBorders'}],],

					[[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Ponto de referência',italics:true}],[{text:pontoReferencia, alignment:'center'}] ] }, layout: 'noBorders'}],
					
					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Data/Hora da infração',italics:true}],[{text:dataHora, alignment:'center'}] ] }, layout: 'noBorders'}], ],
				]
			}
		},
		{
			style: 'tableExample',
			table: 
			{
			    widths: ['*', '*'],
				body: 
				[
					[{colSpan:2, text: '03 - IDENTIFICAÇÃO DA INFRAÇÃO', fillColor: '#b0bec5', border: [false, false, false, true]},''],
					
					[ [{table: {widths: ['*'], heights:['*'],body: [[{text: 'Base legal',italics:true}],[{text: baseLegal, alignment:'center'}] ] },layout: 'noBorders'}] ,

					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Enquadramento',italics:true}],[{text: enquadramento, alignment:'center'}] ] },layout: 'noBorders', border: [false, false, false, true]}, ], ],
				]
			}
		},
		{
			style: 'tableExample',
			table: 
			{
			    widths: ['auto', '*'],
				body: 
				[
					[[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Desdobramento',italics:true}],[{text: desdobramento, alignment:'center'}] ] },layout: 'noBorders'}],
					
					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Descrição da infração',italics:true, }],[{text: descricaoInfracao, alignment:'center'}] ] },layout: 'noBorders'}] ,],

				]
			}
		},
		{
			style: 'tableExample',
			table: 
			{
			    widths: ['*', '*', '*'],
				body: 
				[
					[{colSpan:3, text: '04 - MEDIÇÕES', fillColor: '#b0bec5', border: [false, false, false, true]},'',''],
					
					[[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Limite Regulamentar',italics:true}],[{text: limiteRegulamentar, alignment:'center'}] ] },layout: 'noBorders'}] ,

					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Medição Realizada',italics:true}],[{text: medicaoRealizada, alignment:'center'}] ] },layout: 'noBorders'}], 

					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Medição Considerada',italics:true}],[{text: medicaoConsiderada, alignment:'center'}] ] },layout: 'noBorders'}]],
					
					[[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Excesso Verificado',italics:true}],[{text: excessoVerificado, alignment:'center'}] ] },layout: 'noBorders'}] ,

					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Equipamento Utilizado',italics:true}],[{text: equipamentoUtilizado, alignment:'center'}] ] },layout: 'noBorders'}], 

					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Última Aferição',italics:true}],[{text: ultimaAfericao, alignment:'center'}] ] },layout: 'noBorders'}] ],
				]
			}
		},
		{
			style: 'tableExample',
			table: 
			{
			    widths: ['auto', '*'],
				body: 
				[
					[{colSpan:2, text: '05 - TRANSPORTADOR EMBARCADOR', fillColor: '#b0bec5', border: [false, false, false, true]},''],
					
					[ [{table: {widths: ['*'], heights:['*'],body: [[{text: 'CPF/RG/Outros',italics:true}],[{text: docTransportadorEmbarcador, alignment:'center'}] ] },layout: 'noBorders'}] ,

					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Nome',italics:true}],[{text: nomeTransportadorEmbarcador, alignment:'center'}] ] },layout: 'noBorders'}], ],

				]
			}
		},
		{
			style: 'tableExample',
			table: 
			{
			    widths: ['*'],
				body: 
				[
				   [ [{table: {widths: ['*'], heights:['*'],body: [[{text: 'Assinatura',italics:true}],[{text:'\n', alignment:'center'}] ] },layout: 'noBorders', }] ,],
				]
			}
		},
		{
			style: 'tableExample',
			table: 
			{
			    widths: ['auto','*'],
				body: 
				[
					[{colSpan:2, text: '06 - IDENTIFICAÇÃO DO CONDUTOR', fillColor: '#b0bec5', border: [false, false, false, true]},''   ],
					[ [{table: {widths: ['*'], heights:['*'],body: [[{text: 'CPF',italics:true}],[{text: cpfCondutor, alignment:'center'}] ] },layout: 'noBorders'}] ,

					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Nome',italics:true}],[{text: nomeCondutor, alignment:'center'}] ] },layout: 'noBorders'}], ],

					[[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Habilitação/UF',italics:true}],[{text: `${habilitacaoCondutor}/${ufHabilitacaoCondutor}`, alignment:'center'}] ] },layout: 'noBorders'}],
					
					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Assinatura',italics:true}],[{text:'\n\n', alignment:'center'}] ] },layout: 'noBorders'}] ,],
					]
			}
		},
		
		{
			style: 'tableExample',
			table: 
			{
			    widths: ['*'],
				body: 
				[
					[{colSpan:1, text: '07 - OBSERVAÇÕES', fillColor: '#b0bec5', border: [false, false, false, true]}],
					[{text: ' \n\n'}],
				]
			}
		},
		{
			style: 'tableExample',
			table: 
			{
			    widths: [80,'*'],
				body: 
				[
					[{colSpan:2, text: '08 - AGENTE', fillColor: '#b0bec5', border: [false, false, false, true]},''],
					[ [{table: {widths: ['*'], heights:['*'],body: [[{text: 'Matrícula',italics:true}],[{text:'\n ', alignment:'center'}] ] },layout: 'noBorders'}] ,

					[{table: {widths: ['*'], heights:['*'],body: [[{text: 'Assinatura',italics:true}],[{text:'\n\n', alignment:'center'}] ] },layout: 'noBorders'}], ],
				]
			}
		}
		],
		
	}
	
	pdfMake.createPdf(dd).download(`Auto De Infracao ${numAuto}.pdf`);
}



